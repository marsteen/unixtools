...
//int phonetx(char src[], char dest[], int len, int mode_language);
...
static void trace_info (const char text[], int n, const char err_text[])
/****  output trace info  ****/
{
  const char *s,*s2,*s3;
...
static int phonetx(char src[], char dest[], int len, int mode_language)
 ...
 while ((c = src[i]) != '\0')
   {
    ...
    if (n >= 2)
      ...
      {
          //unsigned char tmpC = (unsigned char) c;
             ...
             if (p0 == 1  &&  z == 0)
               {
                ...
                if (k0 < k)
                  {
                    //strcpy (src+i+k0, src+i+k);  // original                                  
                    /* 
                    // To avoid overflows, [...] the array pointed by destination 
                    // should not overlap in memory with source.
                    char tcc[strlen(src+i+k)+1];
                    strcpy(tcc,src+i+k);
                    strcpy(src+i+k0,tcc);                    
                    */
...
int phonet(const char src_const[], char dest[], int len, int mode_language)
{
	char src[256];
	strcpy(src, src_const);
	return phonetx(src, dest, len, mode_language);
}
